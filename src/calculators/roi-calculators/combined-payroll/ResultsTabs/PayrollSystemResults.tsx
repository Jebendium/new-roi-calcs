...
/**
 * Payroll System Results tab for the Combined Payroll Calculator
 */
const PayrollSystemResults: React.FC<PayrollSystemResultsProps> = ({ 
  ...
  return (
    <div>
      <h3 className="text-xl font-semibold mb-4">Payroll System ROI Analysis</h3>
      <p className="text-slate-600 mb-6">
        This analysis shows the potential return on investment from implementing a new payroll system
        while keeping payroll operations in-house with your own staff.
      </p>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Bar dataKey="roi" fill="#3b82f6" name="Return on Investment" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-collapse">
              ...
              <tbody className="divide-y divide-slate-200">
                ...
                <tr>
                  <td className="px-4 py-3 text-sm text-slate-700 font-medium">Annual Ongoing Costs</td>
                  <td className="px-4 py-3 text-sm text-slate-700 text-right">{formatCurrency(results.annualCosts, 0, 0)}</td>
                </tr>
                <tr className="bg-blue-50">
                  <td className="px-4 py-3 text-sm text-blue-900 font-medium">Efficiency Savings</td>
                  <td className="px-4 py-3 text-sm text-blue-900 text-right">{formatCurrency(results.efficiencySavings, 0, 0)}</td>
                </tr>
                <tr className="bg-green-50">
                  <td className="px-4 py-3 text-sm text-green-900 font-medium">Error Reduction Savings</td>
                  <td className="px-4 py-3 text-sm text-green-900 text-right">{formatCurrency(results.errorReductionSavings, 0, 0)}</td>
                </tr>
                <tr className="bg-green-50">
                  <td className="px-4 py-3 text-sm text-green-900 font-medium">Paper Savings</td>
                  <td className="px-4 py-3 text-sm text-green-900 text-right">{formatCurrency(results.paperSavings, 0, 0)}</td>
                </tr>
                <tr className="bg-amber-50">
                  <td className="px-4 py-3 text-sm text-amber-900 font-medium">Wage Savings</td>
                  <td className="px-4 py-3 text-sm text-amber-900 text-right">{formatCurrency(results.wageSavings, 0, 0)}</td>
                </tr>
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Bar dataKey="costPerPayslip" fill="#6366f1" name="Cost per payslip" />
                </BarChart>
              </ResponsiveContainer>
            </div>
